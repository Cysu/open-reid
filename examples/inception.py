...
def get_data_loaders(data_dir, batch_size, workers):
    ...
    return train_loader, dataset.num_train_ids


def main(args):
    torch.manual_seed(args.seed)

    train_loader, num_train_ids = get_data_loaders(
        args.data_dir, args.batch_size, args.workers)

    model = Inception(num_classes=num_train_ids, num_features=128, dropout=0.5)
